...
describe("Guess the number", () => {
  ...
  let taker1Wallet: any;
  let taker2Wallet: any;

  it("Initialize wallets", async() => {
    const [taker1Wallet, taker2Wallet] = await initializeWallets(connection);
  ...
  it("Set reward by provider", async () => {
  })

  it("Set answer by users", async () => {
  })
