...
async function main() {
  ...
  const myTokenPubkey = await mint.getOrCreateAssociatedAccountInfo(
    myPubkey.publicKey,
  );
  ...
  console.log('\n--- Mint token ---');
  const mint_tx = await mint.mintTo(
    myTokenPubkey.address, // account_pubkey(destination): &Pubkey,
    myPubkey.publicKey,     // owner_pubkey(mint_authority_pubkey): &Pubkey,
    [],                       // signer_pubkeys: &[&Pubkey],
    web3.LAMPORTS_PER_SOL,               // amount: u64,
  );
