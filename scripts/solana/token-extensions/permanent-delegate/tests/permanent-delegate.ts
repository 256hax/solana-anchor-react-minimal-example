...
describe('permanent-delegate', () => {
  ...
  const delegateeKeypair = new anchor.web3.Keypair();
  ...
  it('Create Mint with Permanent Delegate', async () => {
    const transactionSignature = await program.methods
      ...
      .accounts({
        ...
        delegatee: delegateeKeypair.publicKey,
      })
      .signers([mintKeypair, delegateeKeypair])
  ...
  it('Create Token Account, Mint Tokens, and burn with Permanent Delegate', async () => {
    ...
    const transactionSignature = await burnChecked(
      ...
      mintKeypair.publicKey, // Mint Account address
      // delegateeKeypair.publicKey, // Use Permanent Delegate as owner
      randomKeypair.publicKey, // Use Permanent Delegate as owner
      amount, // Amount
      2, // Mint Account decimals
      [delegateeKeypair, randomKeypair], // Additional signers
      undefined, // Confirmation options
      TOKEN_2022_PROGRAM_ID // Token Extension Program ID
    );
    console.log('Your transaction signature', transactionSignature);
